...
Protocol.AbsoluteStartProcess = function(s,arg1, arg2, arg3)
	--下面只处理绝对移动功能
	if s[1] == 0x3F then
		s[5] = bits.rshift(bits.band(arg1,0xff000000), 24);
		s[6] = bits.rshift(bits.band(arg1,0xff0000), 16);
		s[7] = bits.rshift(bits.band(arg1,0xff00), 8);
		s[8] = bits.band(arg1,0xff);
		s[9] = bits.rshift(bits.band(arg2,0xff000000), 24);
		s[10] = bits.rshift(bits.band(arg2,0xff0000), 16);
		s[11] = bits.rshift(bits.band(arg2,0xff00), 8);
		s[12] = bits.band(arg2,0xff);
		s[13] = bits.rshift(bits.band(arg3,0xffff0000), 16);
		s[14] = bits.band(arg3,0xffff);
